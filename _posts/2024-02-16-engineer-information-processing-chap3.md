...
- **개념**
  ...
  - DB 스키마 (스킴)은 데이터 저의 언어라는 특별한 언어로서 표현된 정의의 집합
...
- DB 설계는 개념적 데이터 모델링과 논리적 데이터 모델링을 하는 것
  ...
  - **논리적 데이터 모델링**
    - 개념 세계의 데이터를 DB에 저장할 구조로 표현하는 작ㄱ업
    - 대표적으로 관계 데이터 모델을 이용
...
- **<u>파일 처리 기술</u>**
  ...
  - **색인 순차 파일 <sup>Indexed Sequential File</sup>**
    ...
    - 주어진 키 값 (인덱스) 를 가지고 직접 접근하는 ISAM (Indexed Sequential Access Method), VSAM (Virtual Storage Access Method)
      - **ISAM**: 순차적으로 정렬된 데이터 파일과 키들로 된 인덱스로 구성
        - 데이터 파일 (기본 구욕, 오버플로 구역)과 인덱스 (마스터, 실린더, 트랙) 구성, 정적 인덱스
      - **VSAM**: 데이터 파일과 인덱스 (B+-트리) 는 블록으로 구성

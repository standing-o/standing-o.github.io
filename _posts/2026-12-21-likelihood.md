...
--------------------
> **<u>KEYWORDS</u>**     
> Likelihood, Likelihood란, 가능도, 우도, 우도란, 베이즈 정리, 가우시안 분포, 빈도론, 베이지안
{: .prompt-info }
--------------------
...
- **베이즈 정리 개요(Bayes' Theorem Overview)**
...
- **패턴 인식에의 적용(Application in Pattern Recognition)**
...
### **베이즈 정리 (베이지안 관점) <sup>Bayes' Theorem</sup>**
...
- **최대 우도 추정(Maximum Likelihood Estimation, MLE) - 빈도론적 관점** ㅣ 주어진 데이터의 우도를 최대화하는 파라미터 값을 찾는 것에 중점을 둡니다.
...
- 다차원 벡터 $$x$$에 대해서는 다음과 같이 표현이 변합니다:

$$
  N(x|\mu, \Sigma) = \frac{1}{(2\pi)^{D/2} |\Sigma|^{1/2}} \exp\left\{-\frac{1}{2} (x - \mu)^T \Sigma^{-1} (x - \mu)\right\}
$$

- $$\mu$$는 $$x$$의 차원에 해당하는 평균 벡터입니다.
  - $$\Sigma$$는 변수의 서로 다른 차원이 함께 변하는 방식(상관관계)을 포착하는 공분산 행렬(Covariance Matrix)입니다.
  - $$ㅣ\Sigmaㅣ$$는 공분산 행렬의 행렬식으로, 다차원 공간에서 분포의 부피에 대한 통찰을 제공합니다.
...
- 벡터 $$x$$가 결합 가우시안 분포를 갖는 경우:    
  ...
  - 이 벡터를 두 부분집합 $$x_a$$와 $$x_b$$로 분리하면, 한 집합이 주어졌을 때 다른 집합의 조건부 분포도 가우시안 분포가 됩니다.

- 이 개념을 설명하는 핵심 수식은 다음과 같습니다:

$$x$$를 부분집합 $$x_a$$와 $$x_b$$로 분리합니다:    
...
- $$A$$는 선형 관계를 나타내는 계수 행렬이고, $$b$$는 편향 항(Bias Term)입니다.
  - 벡터 $$z$$에 대한 결합 분포(아래)는 두 분포의 로그를 사용하여 공식화할 수 있습니다:
...
- 공분산 행렬은 다음과 같이 구조화된 정밀도 행렬 $$R$$의 역행렬로 구할 수 있습니다:

![fig3](20251221-3.png){: width="700"}
_정밀도 행렬 R의 구조_

![fig4](20251221-4.png){: width="700"}
_공분산 행렬 유도_



- 결합 분포에서 $$x$$를 적분하여 주변 분포 $$p(y)$$를 유도하는 과정을 설명합니다.
...
- **평균 (Mean)**
  ...
  - $$A$$ ㅣ 평균의 변환을 나타내는 행렬입니다.
    $$\mu$$ ㅣ 확률 벡터의 원래 평균입니다.
    $$b$$ ㅣ 평균에 추가되는 상수 항입니다.
